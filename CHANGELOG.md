...
- **qBittorrent can be reached with an API key instead of a username and password.** qBittorrent 5.2.0 generates one under Options -> Web UI and sends it as a bearer token, so it is not tied to the account you sign in to the Web UI with, and it can be rotated or deleted there without touching your password. **Settings -> Clients -> qBittorrent** gained an **Authentication** choice: pick **API key** and the user and password fields grey out; pick **Username & password** and the key field does. qBittorrent takes one or the other. With a key in play it refuses the login endpoint outright, so only the mode you select is stored, and switching to a key clears the saved password. Existing setups keep working on username and password until you change the mode.
- **A key aimed at a qBittorrent too old to have keys is refused rather than accepted.** Those versions ignore the header the key travels in. If you have also turned authentication off for the address NfoForge connects from -- the usual arrangement for a client on the same machine -- they then answer as though the key had been accepted, and the connection is anonymous with nothing saying so. NfoForge reads the Web API version after connecting and reports what it found, so a key that is doing nothing cannot look like a key that works.
...
- **Plugins can be run from your checkouts instead of being copied in.** Plugins load from your data folder, so developing one meant copying it there after every edit, and the copy is a snapshot -- the first time you forget, you are testing the previous version. `NFOFORGE_DEV_PLUGINS` points NfoForge at a different plugins folder: a folder of checkouts, one directory per plugin, so several can be worked on at once. Name more than one folder by separating them the way `PATH` separates its own. It **replaces** your plugins folder rather than adding to it, which is the rule `NFOFORGE_DATA_DIR` already follows -- a development run is then the same arrangement a real installation has, load order and ID collisions included, rather than a fourth thing that exists nowhere else, and a plugin can never be quietly picked up from a copy you had forgotten was installed. Because the path was typed on purpose and has taken the real folder out of the run, a folder that is missing or holds no plugins is reported in **Settings -> Plugins** rather than passed over -- as is naming a plugin where a folder of plugins belongs, which NfoForge recognises and says so. That screen also names the folders in force, and still names the plugins folder, which **Install** still writes to. A restart is still required, and released builds ignore the variable -- a plugin is trusted Python running inside NfoForge, so the environment does not get to decide what a release imports. See [Working from a checkout](https://jesterr0.github.io/NfoForge/view/plugins/plugin-system.html).
...
- A profile imported alongside a template that had to be renamed is repointed at the template that arrived with it. Left alone it would name the template that was already here, and render a different NFO from the same settings with nothing on screen saying so.
- Settings that named a location on the exporting machine -- the working directory and the paths to FFmpeg, FFprobe, FrameForge and mkbrr -- are cleared on the way in and reported. They are almost never right on another machine, and a path pointing at a drive that is not there fails in the middle of a run rather than at import.
- A bundle written by an older NfoForge is migrated through each schema version as it is imported. One written by a newer NfoForge is refused with a sentence saying so, rather than landing and taking the next launch down. So is a profile that fails validation for any other reason: the whole import stops and nothing is written.
- The import button also accepts a bare `.toml`, because **Save As** produces exactly that and users already have them.
- **An import keeps a copy of its summary**, appended to `logs/migration.log` beside the upgrade's. The window is shown once and nothing on it is rechecked, which is exactly why the upgrade keeps a transcript; an import from Settings changes the data folder the same way and had been leaving no trace of what it renamed.
- The import summary **names plugins an imported profile selects but which are not installed here**. Those capabilities silently fall back to NfoForge's own otherwise -- visible under Settings → Plugins afterwards, but the summary is the moment you are looking. Availability is read from what is on disk rather than from what loaded, so a user with external plugins switched off is not told every selection is missing.
- **Plugins can be installed from Settings.** **Settings -> Plugins** now names the plugins folder, opens it, and offers **Install** from a folder or from a `.zip`. Copying a folder in by hand still works and is still documented; what this adds is the checking. A manifest that is missing or malformed, a module it names that is not there, or an ID a shipped example already uses was only ever discovered on the next launch, in the discovered-plugins table, with the folder already on disk and nothing offering to remove it. All of that is refused before anything is copied, by the loader's own manifest reader rather than a second copy of it. An archive or folder wrapping the plugin one level down -- the shape a downloaded repository unpacks into, and the commonest reason a hand-copied plugin is simply not seen -- is recognised rather than refused.
- **A plugin whose ID is already installed is an update rather than an error.** Refusing it -- which it did at first -- left deleting the folder by hand as the only route, which is the work the installer exists to remove. The dialog says what is being replaced and reads Update; the copy you had is moved into `old_plugins` inside the plugins folder, timestamped, where NfoForge cannot load it but you can still get at it. NfoForge cannot say which of the two is newer: a plugin's version lives in the code it is being asked for permission to run.
- An install is still refused when the module name is already claimed, not only the ID. The loader resolves a module by name and rejects a second one from a different location, and plugins built from the same shipped example start out sharing the example's module name -- so this is the likelier of the two clashes, and the one that reads most like an install that silently did nothing.
- Installing asks first, naming the plugin's ID, its module and where it came from, and repeating that a plugin is trusted code loaded into NfoForge itself. **Nothing is imported or executed while installing** -- the checks read the manifest only -- so a file dialog cannot run someone else's code before it has been accepted. A restart is offered afterwards, since plugins are imported once at startup.
- A plugin copied in this way is filtered the same way the upgrade filters one: virtual environments, `.git`, bytecode, build and coverage output are left behind, while source, tests, documentation and resources come across.
- Archives from anywhere else are treated as untrusted. An entry naming an absolute path, a drive, a parent directory or a symlink is refused outright rather than quietly sanitised, as are archives with implausible entry counts or uncompressed sizes -- and a member that turns out to be larger than its own header claims stops mid-read.
...
- **Test** on the qBittorrent client now says why a login was refused. It reported a bare "Failed" and threw the reason away, so a wrong password, an unreachable host and a rejected key all looked the same.
...
- **The Series Match page picks the TVDB ordering that fits your files** instead of whichever one TVDB happened to serve first. Every ordering is scored on how many of the episodes your filenames claim it actually lists, and on whether each filename's own episode title matches the episode its number lands on; the best fit is pre-selected and each entry in the **TVDB Order** list shows what it was scored on -- `Aired Order — 20 eps · 20/20 matched · titles 100%`. This is the fix for a whole pack coming out misnamed: every ordering of a season has an episode 3, so a release numbered against one binds cleanly to another and renames itself wrong at full confidence with nothing on screen saying so. Picking a different ordering by hand still works, and now re-reads your existing rows against it rather than clearing them.
- **Title matching understands multi-part episodes.** It could only ever answer with a single episode, so a file named after a two-part story -- one carrying no episode numbers at all -- matched half of it and left the other half with no file to give it. Consecutive provider episodes whose titles agree once their part markers are stripped are now treated as one story, and a file named for the story maps to all of it, exactly as a parsed `S01E01-E02` does. A file naming a part still maps to that part: `A.Moral.Star.2` resolves against the story's own episodes rather than by pattern, so a bare trailing number counts as a part where it fits the group, and a title that merely ends in a number -- `Apollo 13` -- is left alone.
- **Numbers and titles are weighed together rather than tried in turn.** Where a filename states a season and episode the selected ordering does not list, its episode title is consulted before the numbers are accepted unverified: a strong match to an episode no other file holds wins and the row reads `title`, while a weak one leaves the parsed numbers standing. The bar is deliberately higher than the general fuzzy threshold, because this overrides a number the filename states plainly. An episode another file already covers is never offered to any matching stage.
- **A new Matched Episode column** naming the episode each row actually resolved to, so a number that looks right while pointing at the wrong episode is visible before anything is renamed.
...
- `&` is read as "and" when comparing titles instead of being dropped, so a provider's `Lost & Found` and a release's `Lost.and.Found` are recognised as the same title rather than as a near miss. Matching also stops reading a release group tag as part of an episode title, which was enough to hide a part number sitting beside it.
- **A season and episode read cleanly from a filename is no longer discarded** when the selected TVDB ordering has no such episode. The row was left blank and the file handed to fuzzy title matching, which in a full pack binds it to an episode another file already holds -- so the pack failed with "Please ensure all files are properly mapped to episodes before continuing" while every cell on screen was filled in. The parsed numbers are kept, marked `parsed (no TVDB match)` and highlighted. Matching can no longer put two files on one episode by any route.
- **A hand-typed correction survives returning to the Series Match page.** Re-entering the page rebuilt the file table with empty cells while its change handler was live, which deleted every row's mapping on the way past, and auto-matching then recomputed the answer that had just been corrected. A mapping already committed -- including one restored with a saved job -- is also adopted on load instead of being rebuilt from scratch.
- **A file covering several episodes marks all of them as mapped.** Only its first episode counted, so a pack with a two-part premiere read "19/20 assigned" with the second part shown unassigned: a gap with no file to fill it and no way to act on it, while validation was in fact already satisfied.
- **A refused mapping names the files.** One sentence covered three different situations and identified nothing; the message now lists the files with no episode number, or the episode two files are fighting over and which files they are, and the first offending row is selected.
- Episode titles are read from the filename rather than from the directories above it. GuessIt was handed the whole path, so for a pack inside a folder carrying its own release info the grandparent was read as the show name and every file's episode title came back as the containing directory's name -- the same wrong value for the whole pack. Season and episode were unaffected, so the damage only showed through fuzzy matching. A season carried only by a `Season NN` parent folder is still found.
- A row whose recorded `episode_end` sits below its episode number claimed no episode at all yet still passed validation. A non-contiguous file recorded as episodes 1 and 5 was read as covering all five by two of the three places that expand a row, which over-claimed three episodes no file held.
- Validation compares the input files one by one rather than by count, so a leftover row for a file no longer in the list cannot make the totals agree while a genuinely unmapped file goes unreported.
...
- Configuration schema 11. A profile that had a Chevereto v3 or v4 site configured keeps it as a labeled site named "Chevereto v3"/"Chevereto v4", and the trackers pointed at it stay pointed at it. A slot never configured migrates to no site at all rather than an empty one.
...
- Now restarts NfoForge when the plugin system is toggled on/off.
- Updated dependencies:
  - platformdirs
