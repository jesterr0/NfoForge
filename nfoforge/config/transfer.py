...
`nfoforge/frontend/windows/config_transfer.py` are a thin layer over these
...
from nfoforge.backend.template_selector import TEMPLATE_SUFFIX
from nfoforge.config.codec import TomlConfigCodec
from nfoforge.config.layout_apply import SUMMARY_LOG_NAME
from nfoforge.config.migrations import document_version, migrate_document
from nfoforge.config.paths import AppPaths
from nfoforge.config.persistence import atomic_write_text
from nfoforge.config.profiles import PROFILE_SUFFIX
from nfoforge.logger.nfo_forge_logger import LOG
from nfoforge.plugins.loader import declared_plugins
from nfoforge.utils.safe_archive import UnsafeArchiveError, read_member, safe_members
from nfoforge.utils.secret_redaction import blank_credentials
...
def _archive(target: Path) -> Path:
    ...
    reaches into the plugin package, which imports back through `nfoforge.config`,
