...
from nfoforge.backend.jobs.values import (
    ENUM_REGISTRY,
    MediaInfoIndex,
    MediaInfoResolver,
    UnencodableValue,
...
def mediainfo_xml(path: Path) -> str:
    ...
    (`nfoforge/backend/media_input.py`), keeping the restored object identical to
    the one it replaces.
...
def mediainfo_sources(context: ProcessingContext) -> dict[Path, MediaInfo]:
    ...
    (`nfoforge/payloads/media_inputs.py`) after a rename: the object's own
