...
EXAMPLE_MEDIA_INPUT_PAYLOAD = MediaInputPayload(
    ...
    # matches the season/episode the settings preview renders with
    # (season_number=1, episode_number=1) so selected-episode tokens resolve
    # to a real value instead of falling through to the TVDB lookup.
    series_episode_map={
        EXAMPLE_FILE_NAME_1: {
            "season": 1,
            "episode": 1,
            "episode_name": "Some episode name 1",
            "episode_data": {
                "seasonNumber": 1,
                "number": 1,
                "absoluteNumber": 1,
...
EXAMPLE_SEARCH_PAYLOAD = MediaSearchPayload(
    ...
    tvdb_id="275274",
    # NOTE: this mirrors the FLAT shape `parse_tvdb_data` in
    # `nfoforge/backend/media_search.py` actually produces (the series/extended
    # object returned by the TVDB API, with `episodes_by_type` merged onto
    # it) -- top-level `episodes`, `episodes_by_type`, `originalLanguage`,
    # `seasons`, and `firstAired`. It must NOT be nested under a "series"
    # key; nothing in production reads it that way, and code that consumes
    # this fixture (the series-management settings preview) expects the
    # flat shape.
    tvdb_data={
        "id": 275274,
        "name": "Series Name",
...
EXAMPLE_SEARCH_PAYLOAD.populate_from_tmdb()
